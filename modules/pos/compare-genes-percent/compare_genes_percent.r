...
mirna_ref.df <- read.table(file= mirna_ref_file, header = T,
  ...
  select(GeneID, miRNA_ID, chrom, prediction_tool)
...
gene_changes.df <- full_join(x = lost_mirna_genes, y = gain_mirna_genes,
                             by = c("GeneID", "miRNA_ID", "chrom", "target"))
...
#                                mutate( GeneID = str_match_all(GeneID, pattern = "ENST00000\\d+\\.\\d" ))

## Get Lost target mirna pairs
lost_genes <- (mirna_ref_intersect.df %>% select(GeneID)) %>% setdiff(mirna_alt_all.df %>% select(GeneID)) %>%
  unique()
...
## Get Gain target mirna pairs
gain_genes <- (mirna_alt_intersect.df %>% select(GeneID)) %>% setdiff(mirna_ref_all.df %>% select(GeneID)) %>%
  unique()
...
## Get remained targets r
remained_genes.df <- (mirna_ref_intersect.df %>% select(GeneID)) %>%  intersect(mirna_alt_intersect.df %>%
