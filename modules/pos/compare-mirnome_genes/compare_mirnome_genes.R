...
Venn_data <- data.frame(  chromosome = c(str_interp("${chromosome}")),
                         remain_genes = c(mirna_alt.v %>%  intersect(mirna_ref.v) %>% length()),
                         lost_genes = c(mirna_ref.v %>%  setdiff(mirna_alt.v) %>%  length()),
                         gain_genes = c(mirna_alt.v %>%  setdiff(mirna_ref.v) %>%  length()))
