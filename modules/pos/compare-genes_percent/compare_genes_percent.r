...
mirna_ref_count.df <- mirna_ref.df %>%  group_by(a_Gene_ID, miRNA_ID) %>% summarise( Number_genes = n())
...
lost_genes <- mirna_ref_intersect.df %>% setdiff(mirna_alt_all.df)

## Get Gain target mirna pairs
gain_genes <- mirna_alt_intersect.df %>% setdiff(mirna_ref_all.df)
...
lost_genes <- lost_genes %>%  mutate(target = "lost") %>% 
  select(-prediction_tool)
gain_genes <- gain_genes  %>%  mutate(target = "gained") %>% 
  select(-prediction_tool)
...
gene_changes.df <- full_join(x = lost_genes, y = gain_genes,
